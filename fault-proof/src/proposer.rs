    ...
    H: OPSuccinctHost + Clone + Send + Sync + 'static,
{
    ...
    async fn fetch_proposer_metrics(&self) -> Result<()> {
        ...
        if let Some(canonical_head_l2_block) = canonical_head_l2_block {
            ...
            // finalized-L2 lookups on the tz path so the metrics loop doesn't error out.
            #[cfg(not(feature = "tz"))]
            {
                // Literal L2 finalized: stays unaffected by L1_BLOCK_TAG so existing dashboards
                // keep their meaning under non-default selection.
                ...
                if let Some(max_provable_l2_block_number) = self
                    ...
                    .await?
                {
                    ProposerGauge::MaxProvableL2BlockNumber
                        .set(max_provable_l2_block_number as f64);
                } else {
