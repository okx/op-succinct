...
interface ITZRootManager {
    /// @notice Records synced roots at their checkpoint height on the target chain. The height must
    ///         be strictly greater than the latest recorded checkpoint.
    ...
    function record(bytes32 withdrawalRoot, bytes32 forceTxRoot, uint256 checkpointBlockHeight) external;
    ...
    function getRoots(uint256 checkpointBlockHeight) external view returns (bytes32 withdrawalRoot, bytes32 forceTxRoot);
