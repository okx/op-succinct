...
# Range split / concurrent proving: split each game's (start, end] into N
# sub-ranges (1-16), each gets its own range proof, then aggregated.
# Default 1 = single-range path equivalent to pre-multi-range behavior.
RANGE_SPLIT_COUNT="${RANGE_SPLIT_COUNT:-1}"
MAX_CONCURRENT_RANGE_PROOFS="${MAX_CONCURRENT_RANGE_PROOFS:-1}"
...
MAX_CONCURRENT_RANGE_PROOFS=$MAX_CONCURRENT_RANGE_PROOFS
